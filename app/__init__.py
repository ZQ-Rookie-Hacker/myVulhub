...
def create_app():
    ...
    # 尝试加载持久化缓存
    logger.info(f"Vulhub 路径: {get_vulhub_path()}")
    logger.info(f"缓存文件: {CACHE_FILE}")

    cached_data = load_persistent_cache()
    if cached_data:
        env_cache.set(cached_data)
        logger.info(f"成功载入持久化缓存，共 {len(cached_data)} 个环境")
    else:
        logger.info("未找到有效缓存，将在首次请求时扫描")

    # 初始化 Docker 操作服务
    ops = VulhubOperations()
    logger.info(f"使用 Docker Compose 命令: {' '.join(ops.compose_cmd)}")
